...
import LoggerTool from '../tool/LoggerTool.js';
import Utilities from '../tool/Utilities.js';
import { Song } from '../../drivers/respositories/database/SongRepository.js';
...
export default class MusicServer {
    ...
    public async establishConnection() {
        ...
        const stageChannel = this.client.channels.cache.get(
            this.stageChannelId,
        );
    ...
    public async addToQueue(
        ...
        if (doesFileExist) {
            ...
            LoggerTool.info(`Path for ${name} is incorrect. (${filename})`);
            return {
                success: false,
    ...
    public async getRecent(limit: number) {
        return this.client.database.guild.getGuildSongHistory(
            this.guildId,
            limit,
        );
    }
    ...
    private async logUsers(song: MusicServerSong) {
        const channel = (
            this.inStageChannel ? this.stageChannel : this.channel
        )!;
